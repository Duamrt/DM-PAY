...
(function() {
  ...
  let DIAS = 30; // janela de projeção
  let SALDO_INICIAL = 0; // captado de localStorage por tenant
  let PAGS = [];
  let RECS = [];
  let CHART = null;
  ...
  async function load() {
    ...
    const fim = isoDay(addDays(HOJE, DIAS));
    const inicio = isoDay(addDays(HOJE, -7)); // pega 7 dias atrás pra mostrar o que aconteceu

    const [pags, recs] = await Promise.all([
      sb.from('payables').select('amount, due_date, paid_at, status, description, suppliers(legal_name)').eq('company_id', COMPANY_ID).gte('due_date', inicio).lte('due_date', fim).limit(2000),
      sb.from('receivables').select('amount, due_date, received_at, status, description, customers(name)').eq('company_id', COMPANY_ID).gte('due_date', inicio).lte('due_date', fim).limit(2000)
    ]);
    PAGS = pags.data || [];
    RECS = recs.data || [];

    // Saldo inicial: localStorage por tenant
    const saldoStored = localStorage.getItem('dmpay-saldo-' + COMPANY_ID);
    SALDO_INICIAL = saldoStored ? parseFloat(saldoStored) : 0;
  }

  function buildSeries() {
    // Para cada dia (HOJE até HOJE+DIAS): entradas, saidas, saldo acumulado
    const labels = [];
    const series = [];
    ...
    for (let i = 0; i <= DIAS; i++) {
      ...
      const entrada = RECS.filter(r => r.due_date === diaIso && r.status !== 'cancelled').reduce((s,r) => s + Number(r.amount), 0);
      const saida = PAGS.filter(p => p.due_date === diaIso && p.status !== 'cancelled').reduce((s,p) => s + Number(p.amount), 0);
      saldo = saldo + entrada - saida;
      labels.push(dia.toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit' }));
      series.push({ dia, diaIso, entrada, saida, saldo, criticoNeg: saldo < 0 });
    }
  ...
  function drawKPIs(series) {
    ...
    const saldoHoje = series[0] ? series[0].saldo : SALDO_INICIAL;
    const saldoFinal = series[series.length-1] ? series[series.length-1].saldo : saldoHoje;
    const piorDia = series.reduce((min, s) => s.saldo < min.saldo ? s : min, series[0] || { saldo: 0, dia: HOJE });
    const diasComMargemBaixa = series.filter(s => s.saldo < 5000).length;
    ...
    if (subs[1]) subs[1].innerHTML = `em ${DIAS} dias`;
  ...
  function drawCriticos(series) {
    ...
    if (!lista) return;
    const criticos = series.filter(s => s.criticoNeg).slice(0, 5);
    if (criticos.length === 0) {
      lista.innerHTML = `<div style="padding:18px;color:var(--text-muted);font-size:12.5px;text-align:center">Nenhum dia crítico nos próximos ${DIAS} dias 🎉</div>`;
      return;
    }
